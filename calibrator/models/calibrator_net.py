...
class CalibratorNet(nn.Module):
    ...
    def setup_net(self):
        ...
        self.pixel_discriminator =  PixelDiscriminator(patch_size=self.patch_size,resolution = self.resolution,cfg = self.cfg)
        init_net(self.pixel_discriminator)                        
        
        if self.task == 'classification':

            self.discriminator = Cls_FeatureDiscriminator(self.cfg)
