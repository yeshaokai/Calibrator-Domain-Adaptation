...
def test(loader_tgt, net):
    net.eval()
    test_loss = 0
    correct = 0
    cm = 0
    for data, target in loader_tgt:
...
def train(loader_src, loader_tgt, net, opt_net, opt_dis, epoch):
    ...
    for batch_idx, ((data_s, _), (data_t, _)) in enumerate(joint_loader):
        ...
        if acc.item() > 0.6:
            ...
            acc = (pred_tgt == label_tgt).float().mean()
            # log net update info
            info_str += " G: {:.3f}".format(loss_gan_t.item())
            info_str += " after_acc : {:0.1f}".format(acc.item() * 100)
        ###########
        # Logging #
        ###########
...
def train_adda(src, tgt, third, model, num_cls, num_epoch=200,
    ...
    test_tgt_data = load_data(tgt, 'test', batch=batch, 
        ...
        image_size=net.image_size, download=True, kwargs=kwargs)
    if third:
        test_third_data = load_data(third, 'test', batch=batch, 
                                    rootdir=join(datadir, third), num_channels=net.num_channels, 
                                    image_size=net.image_size, download=True, kwargs=kwargs)    
    ...
    print ('before all starts, source classifier:')
    print ('{} test'.format(src))
    test(test_src_data,net.src_net)
    print ('{} test'.format(tgt))
    test(test_tgt_data,net.src_net)    
    print ('{} test'.format(third))
    if third:
        test(test_third_data,net.src_net)
        print ('end testing source classifier')
    ...
    for epoch in range(num_epoch):
        ...
        print ('{} test'.format(src))
        test(test_src_data,net.tgt_net)
        print ('{} test'.format(tgt))
        test(test_tgt_data,net.tgt_net)
        if third:
            print ('{} test'.format(third))    
            test(test_third_data,net.tgt_net)
    ...
    outfile = join(outdir, 'adda_{:s}_net_{:s}_{:s}_{:s}.pth'.format(
        model, src, tgt, args['name']))
